...
# Parameters --------------------------------------------------
    ...
    m.pCFPlantLB = en. Param(within = en.NonNegativeReals, initialize = 0.90) 
...
# Cost of unmet of H2 demand - $/kg - default value= 0 as we are not modeling slacks
    ...
    m.pProductionSlackCost = en.Param(within=en.NonNegativeReals, initialize=0, mutable=False)
    
## PV parameters-------------------------------------------------------------
    # PV availability - capacity factor
    m.pPVCapFactor = en.Param(m.t,within=en.NonNegativeReals, 
                              initialize = {tval[j]:pvavailarray[j] for j in range(len(tval))}, mutable=False)
    ...
    m.pGridElecPrice = en.Param(m.t, initialize={tval[j]:pricearray[j] for j in range(len(tval))}, mutable=False)
...
# H2 storage and compressor parameters
    ...
    #m.pH2kgpertank = en.Param(within=en.NonNegativeReals, initialize = 5000000)
...
# H2 compressor,storage and inverter variables --------------------------------
    ...
    # Excess Hydrogen sold in kg/hr.
    m.vH2FlowtoSales = en.Var(m.t, within=en.NonNegativeReals)
    ...
    m.vH2StStatekg = en.Var(m.t, within =en.NonNegativeReals, bounds=(0, None))
...
#  Storage fixed costs in $ - separately evaluate power and energy costs
    def StorFixCost_rule(m):
        return sum(1000*m.pCapCostPowSt[st]*m.vStInstalledMW[st]*m.pCCF+1000*m.pCapCostEnergySt[st]*m.vStInstalledMWh[st]*m.pCCF+m.pFOMCostSt[st]*m.vStInstalledMW[st] for st in m.bes)
...
# H2 storage, compressor and inverter capital costs - Inverter rating set by amount of grid power exports + compressor power required
    def H2StFixCost_rule(m):
        val= m.vCompInstalledMW*m.pCapCostH2Comp*m.pCCF*1000 +m.vCompInstalledMW*m.pFOMCostH2Comp +m.vInvInstalledMW*m.pCapCostInv*m.pCCF*1000 + m.vH2StInstalledNumber*m.pH2kgpertank*m.pCapCostH2st*m.pCCF +m.vH2StInstalledNumber*m.pH2kgpertank*m.pFOMCostH2st
        return val
...
# Revenue from grid electricity sales in $
    def RevenueGridSale_rule(m):
        val = sum(m.vACPowtoGrid[t]*m.pGridElecPrice[t] for t in m.t) + sum(m.vH2FlowtoSales[t] for t in m.t)*15.50
        return val
...
# Balance storage capacity at each time step (kg) - no effiiciency losses assumed
    def H2StSoCBalance(m,t): # wrapping storage capacity to ensure first and last period are matching
        if t in [1]:  # First hour of the yeardefints across the year
            ...
            #return m.vH2flowStorage[t] - m.vH2StflowProd[t] == m.vH2StStatekg[t]
            return m.vH2StStatekg[len(pvavailarray)] + m.vH2flowStorage[t] - m.vH2StflowProd[t] - m.vH2FlowtoSales[t] == m.vH2StStatekg[t]
            
        
        else:
            #return m.vH2StStatekg[t-1] + m.vH2flowStorage[t] - m.vH2StflowProd[t] == m.vH2StStatekg[t]
            return m.vH2StStatekg[t-1] + m.vH2flowStorage[t] - m.vH2StflowProd[t] - m.vH2FlowtoSales[t] == m.vH2StStatekg[t]
...
# H2 plant output generation requirements 
    def H2PlantOutputLB_rule(m,t):
        if en.value(m.pProductionSlackCost) == 0: #- in the absence of slacks
            ...
            return m.vH2PlantOutputSlack[t] <= m.vTotalH2Output[t] + m.vH2FlowtoSales[t] - m.pH2DesignFlowRate*m.pCFPlantLB*m.vProductionCommitment[t]
    ...
    def H2PlantOutputUB_rule(m,t):
        if en.value(m.pProductionSlackCost) == 0: #- in the absence of slacks
            ...
            return m.vH2PlantOutputSlack[t] >= m.vTotalH2Output[t] + m.vH2FlowtoSales[t] - m.pH2DesignFlowRate*m.vProductionCommitment[t]
